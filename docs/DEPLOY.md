...
默认配置：**UTC 每 30 分钟** 触发一次 Workflow；仅当 `stock_list.txt` 在最近 **30 分钟**内有 Git 提交更新时才会真正跑分析；北京时间 **02:00–06:00** 自动跳过。手动触发不受上述限制。

修改频率或时间窗口：编辑 `.github/workflows/daily_analysis.yml` 中的 `schedule` 与判断脚本里的秒数（默认 `1800` = 30 分钟）。
...
| 表达式 | 说明 |
|--------|------|
| `'*/30 * * * *'` | 每 30 分钟（UTC），与默认「30 分钟更新窗口」配套 |
