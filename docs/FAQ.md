...
### Q17: 为什么在 GitHub Actions 仍看到“非交易日跳过”，或本地如何关闭检查？

**GitHub Actions**：`daily_analysis.yml` 已固定 `TRADING_DAY_CHECK_ENABLED=false`，正常不应再出现因交易日历而跳过。若仍看到该提示，请确认运行的是本仓库的「每日股票分析」Workflow，且日志中的环境未覆盖该变量。

**本地 / Docker**：默认启用交易日检查。若需强制执行，可任选其一：

```bash
TRADING_DAY_CHECK_ENABLED=false python main.py
# 或
