...
可配置定时任务自动执行，也可手动触发。本地/ Docker 默认**非交易日不执行**（见下表）；**GitHub Actions** 工作流 `daily_analysis.yml` 在未设置 Repository variable `TRADING_DAY_CHECK_ENABLED` 时**默认关闭交易日检查**，休市日仍会跑股票分析；若希望 CI 也遵守交易日，请在 `Settings → Variables` 中设置 `TRADING_DAY_CHECK_ENABLED=true`。
...
>
> - **环境变量方式**：在 `.env` 或 GitHub Secrets/Variables 中设置；Actions 工作流未配置时默认等价于 `false`（见上文）
> - **UI 勾选方式**：仅在 GitHub Actions 手动触发时可见；勾选后本次带 `--force-run`，与 `TRADING_DAY_CHECK_ENABLED` 叠加时仍为强制执行
